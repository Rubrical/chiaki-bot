...
const execAsync = (command: string) => {
  return new Promise<void>((resolve, reject) => {
    exec(command, (error, stdout, stderr) => {
      if (error) {
        logger.error(`Erro ao executar comando. Código: ${error.code}`);
        logger.error(`Comando stderr: ${stderr}`);
        return reject(error);
      }
      resolve();
    });
...
const toMediaCommand: IChiakiCommand = {
  ...
  async execute(client: ChiakiClient, flag: string[], arg: string, M: SerializedMessage, rawMessage: proto.IWebMessageInfo[]): Promise<void> {
    ...
    try {
      ...
      if (isAnimatedCheck) {
        logger.info("Etapa 1: Convertendo WebP para GIF...");
        const convertCommand = `convert "${inputPath}" "${gifPath}"`;
        await execAsync(convertCommand);

        logger.info("Etapa 2: Convertendo GIF para MP4...");
      ...
      if (isAnimatedCheck) {
        await fs.unlink(gifPath).catch(e => client.log.warn("Falha ao limpar gifPath intermediário"));
      }
    }
  }
}
